...
静态成员变量的值存储在静态全局区，整个程序运行过程中只有一份内存，类似于普通的静态变量，跟普通的静态全局变量不同就是静态成员变量只有通过当前类才能访问，并且静态全局变量还可以设置public、protected、private这种访问权限，这两组不就像是给普通的静态变量加了些访问的约束吗？你可能会问这两组访问约束是怎么实现的？这其实也很简单，无非就是高级语言层面的一些语法约束而已，静态成员变量本质上不就是对应着静态全局区的一块内存嘛，只不过在高级语言层面我们必须通过当前类“类名::静态成员变量”的方式才能访问这块内存，做法其实也很简单，编译器只要发现是个静态成员变量，那编译器在编译到代码里没有通过“类名::静态成员变量”的方式访问这个静态成员变量时就直接编译报错，如果编译到是通过“类名::静态成员变量”的方式访问这个静态成员变量时就编译通过并且翻译成往这块内存里存储或者读取这块内存的汇编就行了，这跟普通变量的访问在本质没什么区别吧，都是操作内存嘛，所谓访问权限其实无非就是编译器在编译高级语言时允不允许某些语法编译通过的问题，这玩意就看编译器怎么设计了，也就是说对于普通变量对应的那块内存，无论你怎么访问编译器都让你编译通过，而对于静态成员变量对应的那块内存，你必须得通过固定的语法编译器才能让你编译通过，于是乎就产生了访问权限，所以访问权限没什么高大上的。
...
静态成员函数存储在代码区，也是只有一份，跟普通函数的区别无非也是只能通过“类名::静态成员函数”来访问这个函数，这种访问权限没什么好说的，无非就是编译器做的一层限制，即编译器一见到“类名::静态成员函数”这样的代码就直接翻译成了“bl 某个静态成员函数的内存地址”这样的汇编，而见到其它访问静态成员函数的代码就一律编译报错，所以也没什么大不了的。编译器见到普通函数的调用不就是一律翻译成“bl 函数的地址”嘛，少了编译报错的一些限制而已。
