...
继承是指子类拥有了父类所有的成员变量、成员函数、静态成员变量（记得补充例子，父类和子类用的其实是同一个变量）、静态成员函数（记得补充例子），是一个针对成员变量、成员函数的特性。用`:`来实现，注意C++里没有规定所有的类都必须继承自某个基类，只要一个类没有父类，那它就是一个基类。
...
我们知道继承子类会拥有父类的成员变量，所以四个类其实是下面这样的：
...
因此我们每创建一个Undergraduates对象其实会占20个字节而非16个字节，这就是多继承带来的菱形继承进而带来的内存冗余的问题，虽然C++又推出了虚继承来解决菱形继承问题，但是那样做又会增加代码的复杂度，因此在C++里还是尽量做到使用单继承，但是不建议使用多继承。

但是可以利用多继承这个语法特性来实现implement多个接口，也就是说把:这个符号看成只有一个在继承、其它都是实现接口就ok了，就跟Dart里的implement一样，实现不照样是类嘛，但是起到的接口的作用，不要真得搞成类与类多继承，这里的例子可以在后面学了抽象类和接口之后补充一下。
...
虚函数跟普通函数一样存储在代码区，虚函数表也是存储在代码区，虚函数表里面存储的是一个一个虚函数的地址、指向一个一个的虚函数，所以虚函数表的本质其实就是一个函数指针数组，而对象内部的虚函数表指针则存储着一个地址、指向代码区虚函数表。
