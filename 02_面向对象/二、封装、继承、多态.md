...
很多编程语言里都不支持多继承，而C++里支持多继承，但是不建议使用，会增加代码的复杂度。在C++里还是尽量做到使用单继承，但是可以利用多继承这个语法特性来实现implement多个接口，也就是说把:这个符号看成只有一个在继承、其它都是实现接口就ok了，就跟Dart里的implement一样，实现不照样是类嘛，但是起到的接口的作用，不要真得搞成类与类多继承，这里的例子可以在后面学了抽象类和接口之后补充一下。
