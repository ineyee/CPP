...
运算符本来是只支持基本数据类型的运算，那如果我们给运算符新增一点功能，比如让加号运算符可以用于对象与对象之间相加、等号运算符可以用于对象与对象之间判断是否相等，或者给左移运算符<<新增打印功能等，这就是运算符重载，运算符重载的本质其实就是一个函数。

> 运算符重载为成员函数的话，运算符左侧的东西必须得是当前类的一个对象，我们只能自定义右侧的东西；运算符重载为全局函数的话，运算符左侧和右侧的东西我们都能自定义，因此：
>
> * 如果要重载的功能跟类紧密相关，那就优先重载为成员函数
>
